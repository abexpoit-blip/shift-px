-- Fix: record_redirect_click was not updating profiles.clicks_used / ours_clicks
-- Result: dashboard quota stuck at 0, quota enforcement bypassed, control panel mismatch
-- This migration re-creates the RPC to also bump profile counters atomically.
...
BEGIN
  ...
  IF _is_bot THEN
    -- Bot: only bump link bot counter (don't touch quota)
    UPDATE public.links
    SET bot_clicks_count = COALESCE(bot_clicks_count, 0) + 1
    WHERE id = _link_id;
  ELSE
    -- Human click: bump link counters
    UPDATE public.links
    ...
    -- NEW: bump profile quota counters so dashboard + quota enforcement work
