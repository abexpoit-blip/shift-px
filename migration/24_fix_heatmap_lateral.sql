-- Fix: "column d does not exist" — the nested subqueries lose the outer
-- generate_series alias. Rewrite heatmap using explicit CROSS JOIN + LEFT JOIN.
...
BEGIN
  ...
  -- Heatmap: build a full 7x24 grid via CROSS JOIN, then LEFT JOIN aggregated clicks.
  WITH click_agg AS (
    SELECT
...
-- Bust the cache so the next read recomputes with the fixed function.
DO $$
BEGIN
