-- =============================================================================
-- Migration 21: Fix countries nested-aggregate bug in _compute_analytics_summary
-- Bug: jsonb_agg(jsonb_build_object(..., COUNT(*) FILTER ...)) is nested.
-- Fix: aggregate per country in inner subquery, then jsonb_agg the rows.
-- =============================================================================
...
BEGIN
  ...
  SELECT
    jsonb_agg(
      (SELECT jsonb_agg(
                COALESCE((SELECT cnt FROM click_agg WHERE day_idx = d AND hour_utc = h), 0)
                ORDER BY h
              )
       FROM generate_series(0, 23) AS h)
      ORDER BY d
    ),
    (SELECT MAX(cnt) FROM click_agg)
  INTO v_heatmap, v_heatmax
  FROM generate_series(0, 6) AS d;

  -- Countries — FIXED: aggregate first, then jsonb_agg
  SELECT jsonb_agg(jsonb_build_object('code', country, 'humans', humans, 'bots', bots))
    INTO v_countries
  FROM (
    SELECT country,
