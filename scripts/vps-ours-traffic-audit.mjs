...
if (!URL_BASE || !KEY) {
  ...
  console.error(
    "   looked in:",
    ENV_FILES.filter((f) => fs.existsSync(f)).join(", ") || "(no env files found)",
  );
  console.error("   url found:", URL_BASE ? "yes" : "NO");
  console.error("   key found:", KEY ? "yes" : "NO");
  console.error(
    "   fix: SUPABASE_URL=http://127.0.0.1:8000 SUPABASE_SERVICE_ROLE_KEY=<key> node scripts/vps-ours-traffic-audit.mjs",
  );
  process.exit(2);
}
...
console.log(
  "  our_adsterra_url   :",
  ourUrl || "❌ NOT SET (ours traffic falls back to safe page!)",
);
console.log("  fallback_url       :", settings?.fallback_url || "-");
console.log(
  "  injection_threshold:",
  settings?.injection_threshold,
  " injection_count:",
  settings?.injection_count,
);
...
if (!ourUrl)
  console.log("  ⚠ FIX: Control Panel → set the Adsterra direct link as our_adsterra_url");
...
for (const [k, v] of tally(clicks, (c) => c.routed_to)) {
  console.log(
    `  ${String(k).padEnd(12)} ${String(v).padStart(6)}  ${pct(v, total)}  ${bar(v, total)}`,
  );
}
...
if (ours.length) {
  ...
  let ourHost = null;
  try {
    ourHost = ourUrl ? new URL(ourUrl).hostname : null;
  } catch {}
  ...
  if (stale)
    console.log(
      `  ℹ ${stale} ours clicks happened BEFORE the link was changed (${changedAt}) — old host is expected.`,
    );
  const mismatched = afterChange.filter(
    (c) => c.signals?.target_host && c.signals.target_host !== ourHost,
  );
  if (ourHost && mismatched.length)
    console.log(
      `  ❌ ${mismatched.length} of ${afterChange.length} post-change "ours" clicks did NOT go to ${ourHost}.`,
    );
  else if (ourHost)
    console.log(`  ✅ all ${afterChange.length} post-change "ours" clicks pointed at ${ourHost}`);
} else {
...
for (const [k, v] of tally(offer, (c) => c.signals?.target_host).slice(0, 10))
  console.log(`    ${k} → ${v}`);
...
for (const [k, v] of tally(clicks, (c) => c.country).slice(0, 12))
  console.log(`  ${String(k).padEnd(6)} ${String(v).padStart(6)} ${pct(v, total)}`);
...
console.log(
  "  devices:",
  tally(clicks, deviceOf)
    .map(([k, v]) => `${k}=${v}`)
    .join("  "),
);

const desktop = clicks.filter((c) => deviceOf(c) === "desktop");
const desktopSafe = desktop.filter(
  (c) => c.routed_to === "safe" || c.routed_to === "fallback",
).length;
const desktopBots = desktop.filter((c) => c.is_bot);
console.log(
  `  desktop: ${desktop.length}, routed to safe/fallback: ${desktopSafe} (${pct(desktopSafe, desktop.length)}), flagged bot: ${desktopBots.length} (${pct(desktopBots.length, desktop.length)})`,
);
if (desktopBots.length) {
  console.log("  desktop bot reasons:");
  for (const [k, v] of tally(desktopBots, (c) => c.bot_reason).slice(0, 10)) {
    console.log(
      `    ${String(k).padEnd(34)} ${String(v).padStart(6)}  ${pct(v, desktopBots.length)}`,
    );
  }
  ...
  console.log(
    `  ⚠ desktop blocked WITHOUT a crawler signature (possible real users lost): ${suspectFalsePositive.length}`,
  );
...
if (fpErr)
  console.log("  ⚠ bot_fingerprints query failed:", String(fpErr.message || fpErr).slice(0, 160));
...
if (fps.length) {
  ...
  if (humanBlocked.length) {
    console.log(
      `  ⚠ ${humanBlocked.length} blocked fingerprints ALSO have human hits (possible false positives):`,
    );
    humanBlocked
      ...
      .forEach((f) =>
        console.log(
          `    ${f.fingerprint_hash} bot=${botCount(f)} human=${humanCount(f)} ${f.last_country ?? ""} ${f.last_ip ?? ""}`,
        ),
      );
    console.log(
      "  → these fingerprints are losing real traffic; raise BOT_BLOCK_THRESHOLD or clear them.",
    );
  } else {
...
if (ourUrl) {
  const UAS = [
    [
      "mobile ",
      "Mozilla/5.0 (Linux; Android 13; SM-A536B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0 Mobile Safari/537.36",
    ],
    [
      "desktop",
  ...
  for (const [label, ua] of UAS) {
    ...
    const verdict = r.loc
      ? `→ redirect ${r.loc.slice(0, 80)}`
      : r.len === 0
        ? "❌ EMPTY BODY (no ad served / rejected)"
        : `body ${r.len}b`;
  ...
  if (proxyBlocked) {
    console.log(
      '  ℹ INCONCLUSIVE — Adsterra answered "Anonymous Proxy detected" for THIS server\'s IP.',
    );
    console.log("     That is a datacenter-IP block on the probe, not proof the link is broken.");
    console.log(
      "     Real mobile users are not affected; test the link from a phone on mobile data instead.",
    );
  } else if (bad) {
...
try {
  const links = await rest(
    "links?select=id,short_code,user_id,adsterra_url,destination_url&limit=5000",
  );
  ...
  console.log(
    hurt.length
      ? `  ⚠ ${hurt.length} users have >10% traffic going to safe page`
      : "  ✅ no user is losing >10% traffic to the safe page",
  );
...
console.log(
  `  ours share: ${oursPct.toFixed(1)}%  |  offer share: ${pct(offer.length, total)}  |  bot share: ${pct(bots, total)}`,
);
if (!ourUrl) console.log("  ❌ our_adsterra_url missing → ours traffic never reaches Adsterra.");
if (ours.length && ourUrl)
  console.log(
    "  ℹ If Adsterra still shows 0, the link itself is the problem (wrong direct-link URL, or Adsterra rejecting the referrer).",
  );
console.log("");
