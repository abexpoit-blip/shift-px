...
if ! sudo ss -tulpn | grep -q ":4000"; then
  ...
  pm2 logs adspx --lines 50 --no-daemon & sleep 5 && kill $!
  exit 1
fi

echo "✅ App is listening on 4000. Updating Nginx..."
sudo sed -i 's/127.0.0.1:3000/127.0.0.1:4000/g' /etc/nginx/sites-available/adspx
sudo sed -i 's/localhost:3000/127.0.0.1:4000/g' /etc/nginx/sites-available/adspx
