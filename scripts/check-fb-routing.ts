...
const FB_UA_RE =
  /facebookexternalhit|facebot|facebookcatalog|facebookplatform|meta-external|meta-webindexer|metainspector|instagram-fbexternalhit/i;
...
const { data, error } = await s
  ...
  .or(
    "bot_reason.ilike.fb-%,ua.ilike.%facebookexternalhit%,ua.ilike.%meta-external%,ua.ilike.%facebot%",
  )
...
console.log(
  `Window: last ${WINDOW_HOURS}h | FB-class hits: ${rows.length} | correct: ${ok} | mismatches: ${mismatches.length}`,
);
...
if (mismatches.length > 0) {
  ...
  mismatches.slice(0, 30).forEach((r: any) => {
    console.log(
      `${r.created_at} | route=${r.routed_to} | reason=${r.bot_reason} | ip=${r.ip} | ua=${(r.ua || "").slice(0, 90)}`,
    );
  });
