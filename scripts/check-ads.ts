...
const { data: clicks } = await s
  .from("clicks")
  .select("routed_to,is_bot,bot_reason")
  .gte("created_at", new Date(Date.now() - 6 * 3600e3).toISOString())
  .limit(5000);
const agg: Record<string, number> = {};
(clicks || []).forEach((c: any) => {
  const k = `${c.routed_to}|bot=${c.is_bot}|${c.bot_reason || "-"}`;
  agg[k] = (agg[k] || 0) + 1;
});
Object.entries(agg)
  .sort((a, b) => b[1] - a[1])
...
const { data: fb } = await s
  .from("clicks")
  .select("created_at,routed_to,bot_reason,country,ua")
  .ilike("bot_reason", "%fb%")
...
const { data: errs } = await s
  ...
  .limit(15);
(errs || []).forEach((e: any) =>
  console.log(e.created_at, "|", e.source, "|", e.message?.slice(0, 120)),
);

console.log("\n=== Recently created links (last 10) ===");
const { data: links } = await s
  .from("links")
  .select("short_code,created_at,clicks_count,bot_clicks_count,is_active,prelanding_template")
  .order("created_at", { ascending: false })
  .limit(10);
(links || []).forEach((l: any) =>
  console.log(
