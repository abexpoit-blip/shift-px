...
export const listShortenerDomains = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("shortener_domains")
      .select(
        "id, domain, dns_target, is_primary, is_active, verified, verified_at, note, created_at",
      )
...
export const addShortenerDomain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase
      .from("shortener_domains")
      .insert({ domain, note: data.note ?? null })
      .select("*")
      .single();
    if (error) throw new Error(error.message);
    return row;
...
export const verifyShortenerDomain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      const r = await fetch(
        `https://cloudflare-dns.com/dns-query?name=${encodeURIComponent(row.domain)}&type=A`,
...
export const deleteShortenerDomain = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabase
      ...
      .maybeSingle();
    if (row?.is_primary)
      throw new Error("Cannot delete the primary domain. Promote another first.");
