...
if [ -n "$DB_CONTAINER" ]; then
  for m in migration/43_*.sql migration/44_*.sql migration/45_*.sql migration/46_*.sql migration/47_*.sql migration/48_*.sql; do
    if [ -f "$m" ]; then
      echo "  Applying $m..."
...
cd "$BUILD_TMP"
npx @tanstack/router-cli generate || true
npm run build
