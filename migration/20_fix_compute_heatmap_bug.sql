-- =============================================================================
-- Migration 20: Fix heatmap query bug in _compute_analytics_summary
-- Bug: subquery references `d` and `h` but only `h` is defined via
--      generate_series(0, 23) h. Missing outer generate_series(0, 6) d.
-- Symptom: ERROR: column "d" does not exist → /analytics returns HTTP 400.
-- =============================================================================
...
BEGIN
  ...
  -- 7d x 24h heatmap — FIXED: outer generate_series(0,6) AS d
  WITH click_agg AS (
    SELECT
